...
def image_to_image(prompt: str, input_image_path: str, s3_key: str):
    local_file_path = download_file(file_url=input_image_path)
    init_image = Image.open(local_file_path)
    init_image = init_image.resize((1024, 1024))
    init_image.save(local_file_path, "png")
    response = requests.post(STABLE_DIFFUSION_XL_1_URL,
                             ...
                             files={
                                 "init_image": open(local_file_path, "rb")
                             },
                             data={
